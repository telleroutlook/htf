...
class TestVariationalBoundNoFlint:
    ...
    def test_raises_import_error_without_flint(self, monkeypatch):
        ...
        from htf.variational import variational_bound, transverse_ising_ham
...
class TestMPSEdgeCases:
    ...
    def _unit_mps(self):
        from htf.mps import MPS
        t0 = np.zeros((1, 2, 1)); t0[0, 0, 0] = 1.0
        t1 = np.zeros((1, 2, 1)); t1[0, 0, 0] = 1.0
        return MPS([t0, t1])
    ...
    def test_apply_gate_three_sites_raises(self):
        from htf.mps import mps_apply_gate, MPS
...
class TestZXSimplificationContinues:
    ...
    def test_spider_fusion_skips_non_spider_nodes(self):
        ...
        g.add_edge(inp, z)
        g.add_edge(z, out)
        g.inputs.append(inp); g.outputs.append(out)
